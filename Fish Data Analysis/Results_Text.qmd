...
contrast_names <- c("CB_ctl-CB_ab","CB_ctl-GD_ctl","CB_ctl-GD_ab","CB_ctl-NT_ctl","CB_ctl-NT_ab",
                   "CB_ab-GD_ctl","CB_ab-GD_ab","CB_ab-NT_ctl","CB_ab-NT_ab",
                   "GD_ctl-GD_ab","GD_ctl-NT_ctl","GD_ctl-NT_ab",
                   "GD_ab-NT_ctl","GD_ab-NT_ab",
                   "NT_ctl-NT_ab")
```
...
polar_no_out <- school_data %>% group_by(Ablation,Species) %>%
                              ...
                              ungroup() |> 
  mutate(logit_polar = logit(School_Polar))
...
#polar_lmer_still <- lm(School_Polar ~ Ablation*Species + Trial, data = school_data)
polar_lmer_logit_still <- lme(logit_polar ~ Ablation*Species, 
                             random = ~1|Trial/Species,
                             correlation = corAR1(form = ~1|Trial/Species), 
                             data = polar_no_out)
polar_s_aov_logit_me <- Anova(polar_lmer_logit_still, type=c("III"))

polar_s_aov_logit_me

em_logit_out <- summary(emmeans(polar_lmer_logit_still, pairwise ~ Ablation*Species))$contrasts$p.value
names(em_logit_out) <- contrast_names
multcompLetters(em_logit_out)
```


```{r}
plot(polar_lmer_logit_still)
qqnorm(resid(polar_lmer_logit_still))
qqline(resid(polar_lmer_logit_still))
```
...
saveRDS(overall_stats_table,"Paper Tables/overall_stats_table.RDS")
...
glmm_school_stats_table <- tibble(Stat = c(rep("NND",3),rep("Polarization",3),rep("Speed",3),rep("Groups",7)),
       ...
       Chisq = round(c(nnd_s_aov_me$`Chisq`[values_to_grab],
                 polar_s_aov_logit_me$`Chisq`[values_to_grab],
       ...
       P_Val = round(c(nnd_s_aov_me$`Pr(>Chisq)`[values_to_grab],
                 polar_s_aov_logit_me$`Pr(>Chisq)`[values_to_grab],
